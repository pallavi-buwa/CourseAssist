            ...
            <div className="space-y-2">
              {node.resources.map((r, i) => (
                <a key={i} href={r.url} onClick={e => e.preventDefault()} className="flex items-center gap-2 text-xs text-gray-300 hover:text-white transition-colors">
                  <span className="text-indigo-400">{RESOURCE_ICONS[r.type]}</span>
        ...
        {mode === 'student' && (
          <Section title="Discover More">
            {!aiResources && !loadingAI && !aiError && (
              <button onClick={handleDiscover} className="w-full bg-indigo-600 hover:bg-indigo-500 text-white rounded-lg px-4 py-2 text-sm font-medium transition-colors">
                Discover more →
              </button>
            )}
            {loadingAI && (
              ...
              <div className="space-y-2">
                {aiResources.map((r, i) => (
                  <a key={i} href={r.url} target="_blank" rel="noreferrer"
                    ...
                    <span className="text-indigo-400 text-base mt-0.5">{RESOURCE_ICONS[r.type]}</span>
