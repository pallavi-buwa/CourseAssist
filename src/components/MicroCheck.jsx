  ...
  return (
    <div className="my-6 rounded-xl border border-gray-700 bg-gray-900 p-4 fade-in">
      <div className="flex items-center gap-2 mb-3">
        <span className="text-xs bg-indigo-600/20 text-indigo-400 border border-indigo-600/30 rounded px-2 py-0.5 font-medium">Quick Check</span>
      </div>
      ...
      {selected !== null && !isCorrect && (
        <div className="mt-3 bg-amber-500/10 border border-amber-500/30 rounded-lg p-3">
          <p className="text-amber-400 text-sm font-medium mb-1">Consider re-reading this section. <button className="underline hover:text-amber-300" onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })}>Re-read ↑</button></p>
          {check.misconception && <p className="text-amber-500/80 text-xs">{check.misconception}</p>}
